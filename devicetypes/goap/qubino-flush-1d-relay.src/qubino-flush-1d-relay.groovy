 ...
 */
metadata {
    definition (name: "Qubino Flush 1D Relay", namespace: "Goap", author: "Kristjan James Caron") {
        capability "Actuator"
        capability "Switch"
        capability "Polling"
        capability "Refresh"
        ...
        attribute "ContactSensor", "boolean" //attribute used to store and display contact sensor status
        ...
        fingerprint mfr:"0159", prod:"0002", model:"0053", deviceJoinName: "Qubino Flush 1D Relay"  //Manufacturer Information value for Qubino Flush 1 Relay
    }
    ...
    tiles(scale: 2) {
        multiAttributeTile(name:"switch", type: "generic", width: 6, height: 4, canChangeIcon: true){
            ...
            tileAttribute ("ContactSensor", key: "SECONDARY_CONTROL") {
                attributeState "true", label:'Door', icon: "st.doors.garage.garage-open", backgroundColor:"#79b821", nextState:"false"
                attributeState "false", label:'Door', icon: "st.doors.garage.garage-closed", backgroundColor:"#ffffff", nextState:"true"
            }
        }
        standardTile("ContactSensor", "device.ContactSensor", decoration:"flat", width: 3, height: 3) {
        	state "true", label:'Open', icon: "st.doors.garage.garage-open", backgroundColor:"#79b821"
            state "false", label:'Closed', icon: "st.doors.garage.garage-closed", backgroundColor:"#ffffff"
        }
        /*
		...
		*/
        ...
        main("switch")
        details(["switch", "ContactSensor", "temperature", "kwhConsumption", "resetPower", "refreshPowerConsumption", "setConfiguration", "setAssociation"])
    }
    preferences {
        /**
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelEndPointReport cmd){
    log.debug "Qubino Flush 1 Relay: firing MultiChannelEndPointReport"
    if(cmd.endPoints > 0){
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd){
    log.debug "Qubino Flush 1 Relay: firing MC Encapsulation event"
...
def zwaveEvent(physicalgraph.zwave.commands.sensorbinaryv2.SensorBinaryReport cmd) {
    log.debug "Qubino Flush 1 Relay: firing sensor report event"
    log.debug cmd
    
    def result = []
    switch(cmd.sensorValue){
        case 0:
        result << createEvent(name:"ContactSensor", value: true, descriptionText:"${device.displayName} is open")
        break;
        case 255:
        result << createEvent(name:"ContactSensor", value: false, descriptionText:"${device.displayName} is closed")
        break;
    }
    return result
